...
library("psych")
colfunc <- colorRampPalette(c("white", "black"))(10)
...
for (GR in Generations){
  ...
  image(GenerationCorr[[GR]],breaks=c(0, 0.2, 0.4, 0.6, 0.8, 0.99, 1.0), col=colfunc[c(1:5,10)], xaxt='n', yaxt='n', main=paste("LD analysis for generation",GR))
  axis(1, at = seq(0, 1, 1/(ncol(GenerationCorr[[GR]])-1)), colnames(GenerationCorr[[GR]]), las=2, cex.axis=0.7)
  axis(2, at = seq(0, 1, 1/(ncol(GenerationCorr[[GR]])-1)), colnames(GenerationCorr[[GR]]), las=2, cex.axis=0.7)
  abline(h=0.0625 + seq(0, 0.875, 1/(ncol(GenerationCorr[[GR]])-1)), col="white")
  abline(v=0.0625 + seq(0, 0.875, 1/(ncol(GenerationCorr[[GR]])-1)), col="white")
  box()
  dev.off()
}
