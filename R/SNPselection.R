...
SNPs_Called_DNAseq <- read.csv("RawData/output.raw.snps.indels.vcf",skip=59,header=TRUE,sep="\t")
...
SNPs_Called_DNAseq <- cbind(SNPs_Called_DNAseq, GT6929 = rep(NA, nrow(SNPs_Called_DNAseq)),GT6954 = rep(NA, nrow(SNPs_Called_DNAseq)))
...
SNPs_Called_DNAseq <- SNPs_Called_DNAseq[-which(SNPs_Called_DNAseq[,"FILTER"] == "LowQual"),c("X.CHROM","POS","ID","REF","ALT","GT6929","GT6954")] # Remove the SNPs with low quality
...
SNPsinBigRegionDNAseq <- SNPsinChr4[-which(SNPsinChr4[,"GT6929"] == "0/0" & SNPsinChr4[,"GT6954"] == "0/0"),]
